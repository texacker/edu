...
%       'specify PDE'-button or selecting Parameters...
%       from the PDE Menu.
%
...
%       $Revision: 1.6 $  $Date: 1996/10/17 21:15:47 $

%Modified by furao@iname.com
thesis_data;
...
if strcmp(action,'initialize')
 ...
 %StrMtx=str2mat('Elliptic','Elliptical Cavity','Axis I','Axis II');
...
elseif strcmp(action,'ok')
  ...
  params=getuprop(pde_fig,'currparam');
  ...
  oldnc=getuprop(pde_fig,'ncafd'); oldnc=oldnc(1);
  appl=getuprop(pde_fig,'application');
  err=0;
  eval('convparams=pdetrans(newparams,appl);','err=1;');
  if err,
    pdetool('error',lasterr);
    return
  end
  ncafd=getuprop(pde_fig,'ncafd'); nc=ncafd(1);
  n=size(convparams,1);
  ...
  if oldnc~=nc & max(nc,oldnc)>4 & nsub<nbt1,
    ...
    ncafd=getuprop(pde_fig,'ncafd'); ncafd(1)=oldnc;
    setuprop(pde_fig,'ncafd',ncafd)
    return;
  end
  ...
  if nsub<nbt1,
    ...
    if max(nc,oldnc)<=4,
      setuprop(pde_fig,'ncafd',[max(nc,oldnc), ncafd(2:4)])
    end
  ...
  setuprop(pde_fig,'currparam',finalparams)
  ...
  if CavityChoice==1
     prompt={'Enter the nodes:',...
             'Enter the alpha:',...
             'Enter the H/R:'};
     def={'13','0.25','0.8'};
     title='Input for Liquid Sloshing(1)';
     lineNo=1;
     ...
     if ~isempty(answer)
        ...
        w0=3;
        G0 = 0;
        nodes=floor(abs(str2num(answer{1}))/2)*2+1;
        alpha=abs(str2num(answer{2}))-floor(abs(str2num(answer{2})));
        H=abs(str2num(answer{3})*R);
     end
     SpheroidDomain = 1;
     RegularMesh = 0;
  elseif CavityChoice==2
     prompt={'Enter the nodes(x):',...
             'Enter the nodes(y):',...
             'Enter the alpha:',...
             'Enter the H/R:'};
     def={'4','9','0.75','0.8'};
     title='Input for Liquid Sloshing(2)';
     lineNo=1;
     answer=inputdlg(prompt,title,lineNo,def);
     if ~isempty(answer)
        Fea_Len=1;
        ...
        w0=3;
        G0 = 0;
        nodes_x=floor(abs(str2num(answer{1})));
        nodes_y=floor(abs(str2num(answer{2}))/2)*2+1;
        alpha=abs(str2num(answer{3}))-floor(abs(str2num(answer{3})));
        H=abs(str2num(answer{4})*R);
     end
     SpheroidDomain = 0;
     RegularMesh = 1;
  else
     prompt={'Enter the nodes:',...
             ...
             'Enter the H/R:'};
     def={'1','0.75','0','0.6'};
     title='Input for Liquid Sloshing(3)';
     lineNo=1;
     answer=inputdlg(prompt,title,lineNo,def);
     if ~isempty(answer)
        Fea_Len=1;
        ...
        M0=1;
        w0=3;
        nodes=floor(abs(str2num(answer{1})));
        alpha=abs(str2num(answer{2}))-floor(abs(str2num(answer{2})));
        G0=abs(str2num(answer{3})*w0^2*Fea_Len);
        H=abs(str2num(answer{4})*R);
        if G0 == 0
           nodes=1;
        end
