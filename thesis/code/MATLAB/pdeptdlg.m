...
%   $Revision: 1.6 $  $Date: 1996/10/17 21:17:47 $
...
if strcmp(action,'initialize')
  ...
  if ~fflag,
    ...
    StrMtx=str2mat(getuprop(pde_fig,'colstring'),...
                   getuprop(pde_fig,'arrowstring'),...
                   getuprop(pde_fig,'deformstring'),...
                   getuprop(pde_fig,'heightstring'));
    user_hndl=zeros(4,1);
    UIPos = UIPos - [0 0.5*Voff+1.5*BWH(2) 0 0];
  ...
  % Set edit fields:
  colstr = getuprop(pde_fig,'colstring');
  arrstr = getuprop(pde_fig,'arrowstring');
  defstr = getuprop(pde_fig,'deformstring');
  htstr = getuprop(pde_fig,'heightstring');
  ...
  timeeigparam=getuprop(pde_fig,'timeeigparam');
...
elseif strcmp(action,'prop_cb'),
  ...
  popstr=fliplr(deblank(fliplr(popupstr(gco))));
...
elseif strcmp(action,'plot')
  ...
  currparams=getuprop(pde_fig,'currparam');
  ...
  % deformed plot (displacements)
    ...
    if isnan(deformdata),
      ...
      p=p+[xadd';yadd'];
      setuprop(pde_fig,'meshdef',[xadd';yadd']);
    end
  end
  ...
  if animate & ~replay & (pde_type==2 | pde_type==3)
    ...
    for j=1:n,
      ...
      caxis([minu maxu]);
      if xy,
        eval(['h=pdeplot(p,e,t,''xydata'',xydata(:,j),''xystyle'',xystyle,',...
  ...
  elseif replay & (pde_type==2 | pde_type==3)
    ...
    if strcmp(zstyle,'off')
      ...
      setuprop(pde_fig,'xydata',xydata);
      setuprop(pde_fig,'xystr',xystr);
    ...
    if pde_type==4 & appl==1 & 1==0
    eval(['pdeplot(t_p,t_e,t_t,''xydata'',xydata,''xystyle'',xystyle,',...
          '''zdata'',zdata,''zstyle'',zstyle,''flowdata'',flowdata,',...
          '''flowstyle'',flowstyle,''colormap'',colormap,',...
          '''intool'',''on'',''xygrid'',xygrid,''mesh'',mesh,',...
          '''title'',title,''contour'',cont,''levels'',lvl)'],'err=1;');
    else
    eval(['pdeplot(p,e,t,''xydata'',xydata,''xystyle'',xystyle,',...
          '''zdata'',zdata,''zstyle'',zstyle,''flowdata'',flowdata,',...
          '''flowstyle'',flowstyle,''colormap'',colormap,',...
          '''intool'',''on'',''xygrid'',xygrid,''mesh'',mesh,',...
          '''title'',title,''contour'',cont,''levels'',lvl)'],'err=1;');
...
elseif strcmp(action,'done')
  ...
  setuprop(pde_fig,'plotflags',[colvar colstyle heightvar heightstyle ...
    vectorvar vectorstyle colval doplot xyplot showmesh animate popval ...
    colflag contflag heightflag vectorflag deformflag deformvar]);
  % [colvar colstyle heightvar heightstyle vectorvar vectorstyle
  % colval doplot xyplot showmesh animationflag popupvalue
  % colflag contflag heightflag vectorflag deformflag deformvar]
  setuprop(pde_fig,'colstring',get(hndls(12),'string'))
  setuprop(pde_fig,'arrowstring',get(hndls(13),'string'))
  setuprop(pde_fig,'deformstring',get(hndls(14),'string'))
  setuprop(pde_fig,'heightstring',get(hndls(15),'string'))
